...
We should not use flags and conditions such as:
	(srv_encrypt_log &&
	 ...
	 get_latest_crypto_key_version() == UNENCRYPTED_KEY_VER)
	...
	byte* key)			/*< out: crypt key*/
{
	...
	if (get_crypto_key(crypt_ver, mysqld_key, MY_AES_BLOCK_SIZE))
	{
		fprintf(stderr,
...
{
	if (!srv_encrypt_log ||
	    (key_ver = get_latest_crypto_key_version()) == UNENCRYPTED_KEY_VER)
	{
